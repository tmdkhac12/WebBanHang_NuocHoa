        ...
        fetch("/backend/api/UserAPI.php?action=login", {
            ...
            .then((data) => {
                ...
                if (data.success) {
                    ...
                    flashErrorMessage(document.querySelector("#login-form #error-message"), data.message);
                }
            })
            .catch((error) => {
                console.log(error)
...
function isEqualsPasswordAndCfPassword() {
    ...
    let confirmPassword = document.querySelector("#register-form #confirmPassword").value;
...
function addRegisterEventHandler() {
    document.querySelector("#register-form").addEventListener("submit", (event) => {
        ...
        if (!isEqualsPasswordAndCfPassword()) {
            flashErrorMessage(error_box, "Mật khẩu và nhập lại mật khẩu chưa trùng khớp.")
            return;
        }
        ...
        let hoten = null;
        let email = null;
        let username = document.querySelector("#register-form #username").value.trim();
        let password = document.querySelector("#register-form #password").value.trim();
        let status = 1;
...
function validateRegisterInput(username, password) {
    ...
    if (!/^[a-zA-Z0-9]+$/.test(username)) {
        errors.push("Username chỉ được chứa chữ và số (không dấu, không ký tự đặc biệt).");
    }
